...
const BootcampSignupPlan = () => {
  return (
    <div className="flex flex-col gap-10">
      <CustomHeading circle>
        <h2>پلن ثبت‌نام</h2>
      </CustomHeading>
      <div className="flex flex-col gap-5 md:flex-row md:h-[360px]">
        <div className="flex overflow-hidden flex-col justify-end items-center bg-primary-base rounded-[30px] shrink-0">
          <Image src={image} width={360} height={360} alt="sign up plan" />
        </div>
        <div className="flex flex-col md:flex-row gap-5 justify-center p-10 h-full bg-primary-base rounded-[30px]">
          {plans.map((item) => (
            <div
              key={item.title}
              className="flex grow flex-col gap-5 p-5 text-center bg-white rounded-[30px]"
            >
