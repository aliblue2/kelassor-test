...
import { getUserInfo, userInfo } from "@/requests/user-panel/getUserInfo";
...
const AccountForm = ({ data }: { data: userInfo }) => {
  ...
  const onSubmit: SubmitHandler<formFields> = async (initialData) => {
    ...
    if (birthdayDay && birthdayMonth && birthdayYear) {
      ...
      Bday = new Date(jalaaliBday.gy, jalaaliBday.gm - 1, jalaaliBday.gd);
    } else {
      if (!data.birthdate) {
  ...
  return (
    <>
      <form
        className="flex py-10 gap-10 max-w-[600px] flex-col grow"
      ...
      >
        ...
        <CustomButton type="submit">ذخیره اطلاعات</CustomButton>
      </form>
    </>
  );
};
